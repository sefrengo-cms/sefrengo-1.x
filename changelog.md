...
Sefrengo v1.4.0-beta2 (01.03.01)
------------------------------------------------------------------------------------------------
...
* REMOVED: FEATURE: loginmodul 1.3.3sf hinzugefügt (Björn)
* REMOVED: FIX:  modulfehler bei array_multisort in der Modulkonfiguration angepasst
* REMOVED: MISC: mod_rewrite_support für dedi erweiterung entfernt
* REMOVED: FIX: reset menuheight in function createMenu() - otherwise in IE a display error occurs, if you call [first] a large menu and at [second] a small menu. (Björn)
* REMOVED: FEATURE: Bildpopupfunktion von Aki und saschapi für wysiwyg2 eingebaut (Björn)
* REMOVED: FIX: CSS-Editor-Unterstützung der Eingabe bei neuen CSS-Regeln (Jürgen)
* REMOVED: FIX: Seiten werden nun richtig angelgt, bei Benutzern, die nicht über das Recht Seiten/ Ordnertemplate konfigurieren, aber über Seiten/ Ordnertemplate auswählen verfügen. Das führte bis hetzt immer zu einer unbrauchbaren Seite. (Björn)
* REMOVED: FIX: HTMLAREA3 - Popupfenster zu klein, buttons/ funktionalität wurde abgeschnitten (Björn)
* REMOVED: FIX: HTMLAREA3 - einige fehlende Sprachstrings übersetzt (Björn)
* REMOVED: FIX: Insideediting Mozilla - editieren bei leeren Contentfeld war nicht möglich (Björn)
* REMOVED: FIX: Bei "wysiwyg mode = 3" wurde opera, etc. nicht berücksichtigt - jetzt fallback auf java* REMOVED: wysiwyg editor kafenio (Björn)
* REMOVED: FEATURE: Htmlarea3 an DeDi Desgin anpassen (Björn)
* REMOVED: FIX: Htmlarea3: deditag* REMOVED: feature attribut "indent" falsch zugeordnet (Björn)
* REMOVED: FEATURE: Sprachstring für HA3 Projekteinstellung hinzugefügt: 3= kein IE* ADDED: Mozilla (Björn)
* REMOVED: FEATURE: Neue Webgrabversion von kfo eingebaut (Björn)
* REMOVED: FEATURE: Modulupdates/ einige Module ugedated, einige neu, alte entfernt (Björn)
  Neue Module:
  ...
  Modulupdates:
    ...
    - Webgrab 1.4
    - Teaserbuilder 1.0.1
* REMOVED: MISC: Modulupdates in SQL eingespielt (Björn)
* REMOVED: MISC: Projektvorlage geupdated, wegen neuer Kafenio Dateien (Björn)
* REMOVED: FEATURE: in einige Verzeichnisse zur Sicherheit .htaccess Dateien mir "Deny from all" Regel hinzugefügt (Björn)
* REMOVED: FEATURE: Setup um Tests bei der Installation erweitert (Reto)
* REMOVED: FEATURE: mip_form Applikation für Dateien hinzugefügt "app_file" (Reto)
* REMOVED: FEATURE: textara-Tag -> transform=bbcode (Reto)
* REMOVED: FEATURE: file-Tag -> filesize, fmtitle, fmdesc, filetype,filename (Reto)
* REMOVED: FEATURE: image-Tag -> filesize, fmtitle, fmdesc, filetype (Reto)
* REMOVED: FIX: typegrop PopUp war fehlerhaft wenn nichts zum bearbeiten war (Reto)
* REMOVED: FIX: falsche Ausgabe beim Image-tag bei Kombination "editbutton" mit menuoptions="false" (Reto)
* REMOVED: FIX: original Bild ausgeben wenn kein Thumb vorhanden (Reto)
* REMOVED: FIX: blank-replace bug behoben (Karsten)
* REMOVED: FEATURE: Kafenio aktualisiert Version 0.8.5 (Reto)
* REMOVED: FIX: Abbruch beim installieren von Plugins (Roland)
* REMOVED: FIX: Thumbnail-Generierung für Option x-fest,y-skaliert bzw. y-fets,x-skaliert (Jürgen)
* REMOVED: FIX: CSS-Editor in französischer Version ohne Funktion wegen fehlender JS-Datei (Jürgen)
* REMOVED: FIX: Image-Anzeige im CSS-Regeleditor, Initalisierung des Editor angepasst (Jürgen)
* REMOVED: Update Projektvorlage (Björn)
* REMOVED: SECUIRTYFIX: Includeangriffe bei HTMLAREA3 (Björn)
* REMOVED: FRATURE: Inlineediting und WYSIWYG (HTMLAREA3) für Mozilla (Björn)
* REMOVED: FIX: Inlineediting Mozilla, Speichernbutton war nicht sichtbar (Björn) 
* REMOVED: FIX: Schreibfehler in der robots.txt (Björn)
* REMOVED: FEATURE: nervige index.html aus dem Projektverzeichnis gekickt (Björn)
* REMOVED: SECURITYFIX: Angriff auf includes mit Variable bei register_globals = on (Björn)
* REMOVED: FEATURE: pagedDoubletAudit (Björn)
* REMOVED: SECURITYFIX: $code konnte manipulliert werden (Björn)
* REMOVED: SECURITYFIX: Variablen in config.php konnte von get/ post/ cookie Parametern überschrieben werden (Björn)
* REMOVED: FIX: mip_form apps cache erzeugte manchmal identische id's (Björn)
* REMOVED: FIX: app groups zeigt nur geuppen an, welche eine id > 3 hatten, jetzt id > 2 (Björn)
* REMOVED: FIX: Tar Archive (extract to lower case) (Roland)
* REMOVED: FIX: Fehlermeldungen Modul/Plugin Upload (Roland)
* REMOVED: FIX: Falsche verzeichnisanzeige nach Bulk-Upload (Jürgen)
* REMOVED: FIX: Titel beim Kopieren von Dateien (Jürgen)
