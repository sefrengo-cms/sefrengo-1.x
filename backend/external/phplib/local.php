...
class DB_cms extends DB_Sql
{
    ...
    function init_cache()
    {
        ...
        $this->cache_groups = (is_array($cfg_client['db_cache_groups'])) ? array_merge($cfg_cms['db_cache_groups'],
            $cfg_client['db_cache_groups']) : $cfg_cms['db_cache_groups'];
        $this->cache_items = (is_array($cfg_client['db_cache_items'])) ? array_merge($cfg_cms['db_cache_items'],
            $cfg_client['db_cache_items']) : $cfg_cms['db_cache_items'];
    }
    ...
    function write_cache()
    {
        ...
        if ($this->cache_id && $this->use_cache) {
            if (!$this->cache_db) {
                $this->cache_db = new DB_cms;
            }
            if ($this->cache_db->read_cache($this->cache_id, true)) {
                $action = $force_overide_cache == true ? 'update' : 'ignore';
            }
    ...
    function delete_cache($Cache_var = false)
    {
        ...
        if ((!$cache_gc || $Cache_var) && $this->use_cache) {
            ...
            if (!$this->garbage_cache($Cache_var)) {
                return;
            }
            $now = date("YmdHis", time());
    ...
    function garbage_cache($force = false)
    {
        ...
        if (($force) || ($cache_gc && $cache_gc < (time() + $this->cache_time)) || (rand(1,
                    100) < $this->cache_gc_probability)) {
            $cache_gc = time();
    ...
    function fetch_query($Sql, $Mode = 'DB_FETCH_ASSOC')
    {
        ...
        while ($this->next_record()) {
            array_push($return, $this->this_record());
        }
...
class cms_CT_Sql extends CT_Sql
{
    ...
    function ac_checkme($id, $name)
    {
        ...
        $ret = true;
        $cquery = sprintf("SELECT count(*) FROM %s WHERE sid='%s' AND NAME='%s'", $cms_db['sessions'], addslashes($id),
            addslashes($name));
        $squery = sprintf("SELECT sid FROM %s WHERE sid  = '%s' AND NAME = '%s'", $cms_db['sessions'], addslashes($id),
            addslashes($name));
        $this->db->query($squery);
        if ($this->db->affected_rows() == 0 && $this->db->query($cquery) && $this->db->next_record() && $this->db->f(0) == 0) {
            // nothing found here
            $ret = false;
        }
    ...
    function ac_sigleme($str, $name, $id)
    {
        ...
        if ($id >= 1 && $this->session_enabled) {
            $this->db->query(sprintf("DELETE FROM %s WHERE NAME = '%s' AND sid != '%s' AND user_id = '%s'",
                $cms_db[sessions], addslashes($name), addslashes($str), addslashes($id)));
        }
    }
    ...
    function ac_sigleid($name, $id)
    {
        ...
        if ($id >= 1) {
            $ret = false;
            $cquery = sprintf("SELECT count(*) FROM %s WHERE user_id='%s' AND NAME='%s'", $cms_db['sessions'],
                addslashes($id), addslashes($name));
            $squery = sprintf("SELECT sid FROM %s WHERE user_id='%s' AND NAME='%s'", $cms_db['sessions'],
                addslashes($id), addslashes($name));
            $this->db->query($squery);
            if ($this->db->affected_rows() == 0 && $this->db->query($cquery) && $this->db->next_record() && $this->db->f(0) == 0) {
                // nothing found here
...
class cms_Frontend_Session extends Session
{
    ...
    function __construct()
    {
        ...
        if ($cfg_client['session_enabled'] == '0') {
    ...
    function auth_validatelogin()
    {
        ...
        if (isset($challenge)) {
            if (!$sess->challenge_me($challenge)) {
                $challengefail = true;
                // Event
                fire_event('login_challenge_fail',
                    array('username' => $username, 'password' => $password, 'challenge' => $challenge));
