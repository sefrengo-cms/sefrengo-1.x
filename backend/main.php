...
include_once(__DIR__ . '/HTML/Template/IT.php');
...
// dB Optimice
if ($cfg_cms['db_optimice_tables']['enable'] && (time(
        ) > ($cfg_cms['db_optimice_tables']['last_run'] + $cfg_cms['db_optimice_tables']['time']))) {
    lib_optimice_tables();
    $val_ct->set_value(
        array('group' => 'cfg', 'client' => 0, 'key' => 'db_optimice_tables', 'key2' => 'last_run', 'value' => time())
    );
}
...
$pos = strpos($area, '_');
$allowed_area = ($pos) ? substr($area, 0, $pos) : $area;
...
$rep = new repository;
// Run init Plugins
if ($cfg_rep['repository_init_plugins']) {
    $rep->init_plugins();
}
...
if (is_array($cfg_cms['autostart']['backend'])) {
    foreach ($cfg_cms['autostart']['backend'] as $value) {
...
if (($cfg_cms['gzip'] == '1') && str_contains($ACCEPT_ENCODING, "gzip") && !headers_sent()) {
