...
class core_bbcode
{
    ...
    {
        if (!array_key_exists($aliasof, $this->tags) or array_key_exists($name, $this->tags)) {
            return false;
        }
    ...
    {
        if (!is_array($params)) {
            return 'Paramater array not an array.';
        }
        if (!array_key_exists('Name', $params) or empty($params['Name'])) {
        ...
        if (!array_key_exists('HtmlBegin', $params)) {
            return 'HtmlBegin paremater not specified!';
        }
        ...
        if (array_key_exists($params['Name'], $this->tags)) {
            return 'The name you specified is already in use.';
        }
    ...
    function parse_bbcode($text)
    {
        foreach ($this->tags as $tagname => $tagdata) {
            ...
            $replace = str_replace(array('%%P%%', '%%p%%'), '%%MYSEARCH%%', '\'' . $tagdata['HtmlBegin'] . '\'');
            //            $text = preg_replace($startfind.'e',$replace,$text);
            $text = preg_replace_callback($startfind, function ($matches) use ($replace, $tagdata) {
                $param = $this->onparam($matches[1], $tagdata['ParamRegexReplace']);
                $replace = str_replace('%%MYSEARCH%%', $param, $replace);
