...
class values_ct
{
    ...
    function set_value($mixed)
    {
        ...
        $sql_client = (empty($mixed['client'])) ? '' : 'AND idclient IN (' . (int)$mixed['client'] . ')';
        $sql_lang = (empty($mixed['lang'])) ? '' : 'AND idlang IN (' . (int)$mixed['lang'] . ')';
        ...
        $sql_id = (empty($mixed['id'])) ? "" : "AND V.idvalues = '" . (int)$mixed['id'] . "' ";
