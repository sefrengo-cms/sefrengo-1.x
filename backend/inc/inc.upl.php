...
function show_files($idopen) {
	...
	$is_detailview = ($viewtype == 'detail');	// unterscheidung f�r die anzeige
	...
	while($db->next_record()) {
		...
		$idfile = $db->f('idupl');
	    // pr�fe ob Datei gezeigt werden darf ...
		...
		// create view of file // Datei in Originalgr��e anzeigen
...
function showDirectory( $level, $line_array, $prefixexpand, $status, $monocolor = false ) {
    ...
    // Auswahl f�r Detail- oder Compact-Modus
		if ($viewtype == 'compact')
