    ...
    function make($File, $LangArray = '')
    {
        ...
        if (is_array($TemplateLoopArray)) {
            ...
            for ($f = 0; $f < $counter; $f++) {
                ...
                $counter = count($KeysLoopValue);
                for ($t = 0; $t < $counter; $t++) {
                    $Loopb = $Loop;
                    $counter = count($KeysLoopTemplate);
                    for ($s = 0; $s < $counter; $s++) {
                        $Loopb = str_replace(
                            "<!--{" . $KeysLoopTemplate[$s] . "}-->",
                            $TemplateLoopArray[$KeysLoopname[$f]][$KeysLoopTemplate[$s]][$t],
                            $Loopb
                        );
                    }
                    $LoopFinal .= $Loopb;
...
class setup
{
    ...
    public $globals;
    public $debug = false;
    public $mysql_con_handle;
    public $updatefiles = array(
        'updates.sql',
        ...
        'updates_from.01.06.08.sql',
        'updates_from.01.06.09.sql'
    );
    ...
    function __construct()
    {
        ...
        if (isset($this->globals['mode'], $this->globals['action'])
            && $this->globals['mode'] === 'update' && $this->globals['action'] === 'enter_email'
        ) {
            $this->globals['action'] = 'screen_execute_update_and_finish';
        }
    }
     ...
     */
    function make_setup(): string
    {
        $return_this = '';

        /* @var array $cms_lang */
        /* @var array $template_lang */
        if (isset($this->globals['action'])) {
            if ($this->globals['action'] !== '') {
                include_once('templates/lang/' . $this->globals['lang'] . '.php');
            } else {
                include_once(__DIR__ . '/templates/lang/de.php');
            }
    ...
    function screen_pretest()
    {
        ...
        $tpl->insert(
            'test_PHP_check',
            'value_PHP_check',
            $this->test_config(version_compare(phpversion(), '5.6.0') <= 0 ? 0 : 1)
        );
        $tpl->insert(
            'test_PHP_check',
            'class_PHP_check',
            $this->test_config_text(version_compare(phpversion(), '5.6.0') <= 0 ? 0 : 1)
        );
        $tpl->insert(
            'test_PHP_check',
            'style_PHP_config',
            $this->test_config_style(version_compare(phpversion(), '5.6.0') <= 0 ? 0 : 1)
        );
        ...
        $php_settings = array(
            array('Safe Mode', 'safe_mode', 'OFF', $this->cms_lang['pretest_safe_mode']),
        ...
        foreach ($php_settings as $phpset) {
            ...
            $tpl->insert(
                'test_PHP_config',
                'class_PHP_config',
                $this->test_config_text($this->test_get_php_config_value($phpset[1]) == $phpset[2])
            );
            $tpl->insert(
                'test_PHP_config',
                'style_PHP_config',
                $this->test_config_style($this->test_get_php_config_value($phpset[1]) == $phpset[2])
            );
        ...
        if (file_exists("../$folder")) {
            ...
            if ($is_unix === 0) {
                ...
                $tpl->insert(
                    'test_folder',
                    'value_folder',
                    '<p><a href="#" onclick="javascript:sf_reload()">' . $this->cms_lang['again'] . '</a></p><img src="templates/img/warning.gif" alt="warning" title="warning" />'
                );
    ...
    function screen_test()
    {
        ...
        if (@file_exists('../backend/inc/config.php')) {
            ...
            $tpl->insert(
                'test_configuration',
                'value_configuration',
                '<img src="templates/img/ok.gif" alt="ok" title="ok" />'
            );
    ...
    function screen_enter_path()
    {
        ...
        //make pathvariables
        if (!$_SERVER['PHP_SELF']) {
            $_SERVER['PHP_SELF'] = $this->globals['PHP_SELF'];
        }
        if (!$_SERVER['HTTP_HOST']) {
            $_SERVER['HTTP_HOST'] = $this->globals['HTTP_HOST'];
        }
    ...
    function screen_validate_path()
    {
        ...
        if ($error) {
            $tpl->insert('', 'next_step', 'screen_validate_path');
            $target_tpl = "enter_path.tpl";
        } elseif (($this->globals['sql_target'] == "updates.sql") || ($this->globals['sql_target'] == "backup.sql")) {
    ...
    function validate_admin_pass()
    {
        $error = false;
        $tpl = new gb_template();
        if (empty($this->globals['adminpass']) || $this->globals['adminpass'] != $this->globals['adminpass1'] || preg_match(
                '/[\'\"\#\<\>]/i',
                $this->globals['adminpass']
            )) {
    ...
    function screen_enter_mysql_data()
    {
        ...
        $this->globals['mode'] = $this->globals['sql_target'] == 'updates.sql' ? 'update' : 'normal';
        $tpl->insert('', 'mode', $this->globals['mode']);
        $tpl->insert('', 'sql_target', $this->globals['sql_target']);
        $tpl->insert('', 'version', $this->version_text);
        $tpl->insert('', 'lang', $this->globals['lang']);
    ...
    function screen_ready_to_insert_sql_dump()
    {
        ...
        if (empty($con_handle)) {
            ...
            $tpl->insert(
                '',
                'connection_error',
                $this->cms_lang['connection_error3'] . $this->globals['db'] . $this->cms_lang['connection_error4']
            );
            $tpl->insert('', 'connection_class', 'warning');
            $error = true;
        }
        ...
        if ($error) {
            ...
            if ($this->globals['sql_target'] == 'backup.sql') {
            ...
            } elseif ($this->globals['sql_target'] == 'updates.sql') {
                //Update
                $tpl->insert('', 'prefix', $this->globals['prefix']);
                $target_tpl = "ready_to_insert_sql.tpl";
                ...
                if (!is_array($arr_version)) {
                    ...
                    $tpl->insert('', 'next_step', 'screen_ready_to_insert_sql');
                    $target_tpl = "enter_mysql_data.tpl";
                } elseif ($this->compare_current_version_with('01', '01', '90') == 'higher') {
                    $tpl->insert('', 'prefix', $this->globals['prefix']);
    ...
    function screen_check_version()
    {
        ...
        print_r(
            $this->compare_current_version_with($this->version['prior'], $this->version['minor'], $this->version['fix'])
        );
        echo "<hr>";

        if ($this->compare_current_version_with(
                $this->version['prior'],
                $this->version['minor'],
                $this->version['fix']
            ) == 'higher') {
    ...
    function screen_utf8_convert()
    {
        ...
        $tpl->insert(
            '',
            'converter_url',
            'tools/utf8_converter.php?action=start_from_external_config&amp;data=' . $config_string
        );
        $tpl->insert('', 'prefix', $this->globals['prefix']);
        $tpl->insert('', 'next_step', 'screen_download_config');
    ...
    function compare_current_version_with($prior, $minor, $fix)
    {
        ...
        if ($versionarray['prior'] > $prior) {
            return 'lower';
        } elseif ($versionarray['prior'] == $prior) {
            if ($versionarray['minor'] > $minor) {
                return 'lower';
            } elseif ($versionarray['minor'] == $minor) {
    ...
    function screen_download_config()
    {
        ...
        if ($this->globals['mode'] == 'update') {
            $tpl->insert('', 'show_txt_only_by_update', $this->cms_lang['show_txt_only_by_update']);
        } else {
    ...
    function insert_config_values()
    {
        ...
        //DEBUGGING
        if ($this->debug) {
            echo "Auszuf&#252;hrende Config Values querys:  $sql_count <br><br>";
        }
        ...
        for ($i = 0; $i < $sql_count; $i++) {
            ...
            if ($sql !== '' && $sql !== '0') {
                ...
                //DEBUGGING
                if ($this->debug) {
                    if (mysqli_error($con_handle) != '') {
                        echo $i + 1 . ":  <font color='darkred'><b>FEHLER</b></font>  -->  " . mysqli_error(
                                $con_handle
                            ) . "<br>" . $sql . '<br><br>';
                    } else {
    ...
    function manage_insert_sql_dump()
    {
        if ($this->globals['sql_target'] == 'updates.sql') {
            ...
            if ($versionarray['prior'] > $fileversion['1']) {
                return;
            } elseif ($versionarray['prior'] == $fileversion['1']) {
                if ($versionarray['minor'] > $fileversion['2']) {
                    return;
                } elseif ($versionarray['minor'] == $fileversion['2']) {
            ...
            for ($i = $key; $i < $filecount; $i++) {
                ...
                if (is_file('sql/' . $basefile . '_client.sql')) {
                    $result = mysqli_query(
                        $this->mysql_con_handle,
                        "SELECT idclient FROM " . $this->globals['prefix'] . 'clients'
                    );
                    if (!$result) {
                ...
                if (is_file('sql/' . $basefile . '_lang.sql')) {
                    $result = mysqli_query(
                        $this->mysql_con_handle,
                        "SELECT idclient, idlang FROM " . $this->globals['prefix'] . "clients_lang"
                    );
                    if (!$result) {
    ...
    function insert_sql_dump($special_replace = false)
    {
        ...
        $sql_data = implode(
            '',
            (file(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'sql' . DIRECTORY_SEPARATOR . $this->globals['sql_target']))
        );
        //utf8 encode
        $sql_data = utf8_encode($sql_data);
        ...
        // DEBUGGING
        if ($this->debug) {
            echo $this->globals['sql_target'] . " - Auszuf&#252;hrende Dump querys:  $sql_count <br><br>";
        }
        for ($i = 0; $i < $sql_count; $i++) {
            $sql = trim($sql_pieces[$i]);
            if ($sql !== '' && $sql !== '0') {
                ...
                //DEBUGGING
                if ($this->debug) {
                    if (mysqli_error($this->mysql_con_handle) != '') {
                        echo $i + 1 . ":  <font color='darkred'><b>FEHLER</b></font>  -->  " . mysqli_error(
                                $this->mysql_con_handle
                            ) . "<br>" . $sql . '<br><br>';
                    } else {
                        echo $i + 1 . ":   <font color= 'darkgreen'><b>AUSGEF&#220;HRT</b></font><br>";
                    }//. $sql . '<br><br>';
                }
            }
        }
    ...
    {
        if (empty($this->globals['host'])) {
            die('Fatal error - No DB connection data');
        }
